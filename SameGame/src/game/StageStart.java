...
public class StageStart extends GameProcess {
	private Map map;
	public StageStart(Map map){
		this.map = map;
	}
	public GameProcess update(){
		MouseFacade mf = Controller.getMouseFacade();
//		if(mf.getMouseLeftPressCount() == 1) return new GamePlay(this.map);
		return null;
	}
	...
	public void draw(){
		...
		dLib.drawString(200, 240, "ゲームスタート！", new Color(0,0,0), FontMgr.getInstance().getFontToId(FontMgr.FontId.POPMENU), true);
